...
class BaseRecognizer_fusion(BaseModel, metaclass=ABCMeta):
    """Base class for recognizers.
    ...
    def __init__(self,
        ...
        # Record the source of the backbone.
        self.backbone_from = 'mmaction2'
        if is_from(backbone, 'mmcls.') or is_from(backbone_gcn, 'mmcls.'):
            try:
                # Register all mmcls models.
                import mmcls.models  # noqa: F401
            except (ImportError, ModuleNotFoundError):
                raise ImportError('Please install mmcls to use this backbone.')
            self.backbone = MODELS.build(backbone)
            self.backbone_gcn = MODELS.build(backbone_gcn)
            self.backbone_from = 'mmcls'
        elif is_from(backbone, 'mmpretrain.') or is_from(backbone_gcn, 'mmpretrain.'):
            try:
                # Register all mmpretrain models.
                import mmpretrain.models  # noqa: F401
            except (ImportError, ModuleNotFoundError):
            ...
            self.backbone_gcn = MODELS.build(backbone_gcn)
            self.backbone_from = 'mmpretrain'
        elif is_from(backbone, 'torchvision.') or is_from(backbone_gcn, 'torchvision.'):
            try:
                import torchvision.models
            except (ImportError, ModuleNotFoundError):
                raise ImportError('Please install torchvision to use this '
            ...
            self.backbone_gcn = MODELS.build(backbone_gcn)
            self.feature_shape = backbone.pop('feature_shape', None)
            backbone_pose_type = backbone.pop('type')
            backbone_gcn_type = backbone_gcn.pop('type')
            if isinstance(backbone_pose_type, str):
                backbone_pose_type = backbone_pose_type[12:]
                self.backbone = torchvision.models.__dict__[backbone_pose_type](
                    **backbone)
            if isinstance(backbone_gcn_type, str):
                backbone_gcn_type = backbone_gcn_type[12:]
                self.backbone_gcn = torchvision.models.__dict__[backbone_gcn_type](
                    **backbone_gcn)
            else:
                self.backbone = backbone_pose_type(**backbone)
                self.backbone_gcn = backbone_gcn_type(**backbone_gcn)
            # disable the classifier
            self.backbone.classifier = nn.Identity()
            self.backbone.fc = nn.Identity()
            self.backbone_gcn.classifier = nn.Identity()
            self.backbone_gcn.fc = nn.Identity()
        elif is_from(backbone, 'timm.') or is_from(backbone_gcn, 'timm.'):
            # currently, only support use `str` as backbone type
            try:
                import timm
            except (ImportError, ModuleNotFoundError):
                raise ImportError('Please install timm>=0.9.0 to use this '
                                  'backbone.')
            self.backbone_from = 'timm'
            self.pose_feature_shape = backbone.pop('feature_shape', None)
            self.gcn_feature_shape = backbone_gcn.pop('feature_shape', None)
            # disable the classifier
            backbone['num_classes'] = 0
            backbone_gcn['num_classes'] = 0
            backbone_pose_type = backbone.pop('type')
            backbone_gcn_type = backbone_gcn.pop('type')
            if isinstance(backbone_pose_type, str):
                backbone_pose_type = backbone_pose_type[5:]
                self.backbone = timm.create_model(backbone_pose_type, **backbone)
            if isinstance(backbone_gcn_type, str):
                backbone_gcn_type = backbone_gcn_type[5:]
                self.backbone_gcn = timm.create_model(backbone_gcn_type, **backbone_gcn)
            else:
                raise TypeError(
                    f'Unsupported timm backbone type: {type(backbone_pose_type)}')
        else:
            self.backbone = MODELS.build(backbone)
            self.backbone_gcn = MODELS.build(backbone_gcn)
    ...
    def init_weights(self) -> None:
        ...
        if self.backbone_from in ['torchvision', 'timm']:
            ...
            # avoid repeated initialization
            self.backbone.init_weights = fake_init
            self.backbone_gcn.init_weights = fake_init
        super().init_weights()
