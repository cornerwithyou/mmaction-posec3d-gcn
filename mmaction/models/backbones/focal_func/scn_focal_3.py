...
class SparseBlock_input(spconv.SparseModule):
    def __init__(
        ...
        self.conv1 = spconv.SubMConv3d(17,32,bias=False,indice_key=indice_key,)
...
class SparseBlock(spconv.SparseModule):
    def __init__(
        ...
        self.conv1 = spconv.SubMConv3d(in_channels, mid_channels,kernel_size=3,stride=1,padding=1, bias=False,indice_key=indice_key[0],)
        self.bn1 = build_norm_layer(norm_cfg, mid_channels)[1]
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = spconv.SubMConv3d(mid_channels, mid_channels,kernel_size=3,stride=1,padding=1, bias=False,indice_key=indice_key[1])
        self.bn2 = build_norm_layer(norm_cfg, mid_channels)[1]
        self.conv3 = spconv.SubMConv3d(mid_channels, out_channels,kernel_size=3,stride=1,padding=1, bias=False,indice_key=indice_key[2])
        self.bn3 = build_norm_layer(norm_cfg, out_channels)[1]
        if downsample:
            self.downsample = spconv.SubMConv3d(in_channels, out_channels, kernel_size=3,stride=1,padding=1,bias=False,indice_key=indice_key[3])
    ...
    def __init__(
        ...
        self.conv1 = SparseSequentialBatchdict(
            SparseBlock(in_channels=32,mid_channels=32,out_channels=128,
                        downsample=True,norm_cfg=norm_cfg, indice_key="res1"),
            SparseBlock(in_channels=128,mid_channels=32,out_channels=128,
                        downsample=False,norm_cfg=norm_cfg, indice_key="res1"),
            SparseBlock(in_channels=128,mid_channels=32,out_channels=128,
                        downsample=False,norm_cfg=norm_cfg, indice_key="res1"),
            SparseBlock(in_channels=128,mid_channels=32,out_channels=128,
                        downsample=False,norm_cfg=norm_cfg, indice_key="res1"),
        ...
        self.conv2 = SparseSequentialBatchdict(
            SparseBlock(in_channels=128,mid_channels=64,out_channels=256,
                        downsample=True,norm_cfg=norm_cfg, indice_key="res2"),
            SparseBlock(in_channels=256,mid_channels=64,out_channels=256,
                        downsample=False,norm_cfg=norm_cfg, indice_key="res2"),
            SparseBlock(in_channels=256,mid_channels=64,out_channels=256,
                        downsample=False,norm_cfg=norm_cfg, indice_key="res2"),
            SparseBlock(in_channels=256,mid_channels=64,out_channels=256,
                        downsample=False,norm_cfg=norm_cfg, indice_key="res2"),
        ...
        self.conv3 = SparseSequentialBatchdict(
            SparseBlock(in_channels=256,mid_channels=128,out_channels=512,
                        downsample=True,norm_cfg=norm_cfg, indice_key="res3"),
            SparseBlock(in_channels=512,mid_channels=128,out_channels=512,
                        downsample=False,norm_cfg=norm_cfg, indice_key="res3"),
            SparseBlock(in_channels=512,mid_channels=128,out_channels=512,
                        downsample=False,norm_cfg=norm_cfg, indice_key="res3"),
        )
