...
def conv3x3(in_planes, out_planes,stride=1, indice_key=None,norm_cfg=None ,  bias=True,convtype="sps",**kwargs):
    ...
    if convtype == "sub":
        ...
        return spconv.SparseConv3d(in_planes,out_planes,kernel_size=(1,3,3),stride=(1,1,1),padding=(0,1,1),bias=bias)
    else:
...
def conv311(in_planes, out_planes,stride=1, indice_key=None,norm_cfg=None ,  bias=True,convtype="sps",**kwargs):
    if convtype == "sub":
        return spconv.SubMConv3d(in_planes,out_planes,kernel_size=(3,1,1),stride=stride,padding=1,bias=bias,indice_key=indice_key,)
    elif convtype == "sparse":
        return spconv.SparseConv3d(in_planes,out_planes,kernel_size=(3,1,1),stride=(1,1,1),padding=(1,0,0),bias=bias,)
    else:
        return call_Sps(in_planes,out_planes, kernel_size=(3,1,1),stride=stride, padding=1,indice_key=indice_key + "sps311",)

def conv1x1(in_planes, out_planes,stride=1, indice_key=None,norm_cfg=None ,  bias=True,convtype="sps",**kwargs):
    """1x1 convolution"""
    if convtype == "sub":
        return spconv.SubMConv3d(in_planes,out_planes,kernel_size=(1,1,1),stride=stride,padding=1,bias=bias,indice_key=indice_key,)
    elif convtype == "sparse":
        return spconv.SparseConv3d(in_planes,out_planes,kernel_size=(1,1,1),bias=bias,)
    else:
...
class SparseBasicBlock2(spconv.SparseModule):
    def __init__(
        ...
        self.bn3 = build_norm_layer(norm_cfg, planes)[1]
        if downsample or inplanes != planes:
            self.downsample = conv1x1(inplanes, planes, indice_key=indice_key + "3",norm_cfg=norm_cfg,convtype=convtype[0], bias=bias)
...
class spsresnet(nn.Module):
    def __init__(
        self, num_input_features=128, norm_cfg=None, conv1="spss",conv2="spss",conv3="spss", **kwargs
    ):
