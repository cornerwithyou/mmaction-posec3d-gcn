...
def conv3x3(in_planes, out_planes,norm_cfg=None , stride=1, indice_key=None, bias=True,**kwargs):
    """3x3 convolution with padding"""
    return FocalSparseConv(in_planes,out_planes,kernel_size=(1,3,3),voxel_stride=1,norm_cfg =norm_cfg ,indice_key=indice_key,
                           skip_loss=False, enlarge_voxel_channels=-1,
                            mask_multi=True, topk=True, threshold=0.5)


def conv311(in_planes, out_planes,norm_cfg=None , stride=1, indice_key=None, bias=True,**kwargs):
    return FocalSparseConv(in_planes,out_planes,kernel_size=(3,1,1),voxel_stride=1,norm_cfg =norm_cfg ,indice_key=indice_key,
                           skip_loss=False, enlarge_voxel_channels=-1,
                            mask_multi=True, topk=True, threshold=0.5)


def conv1x1(in_planes, out_planes, norm_cfg=None ,stride=1, indice_key=None, bias=True,**kwargs):
    """1x1 convolution"""
    return FocalSparseConv(in_planes,out_planes,kernel_size=(1,1,1),voxel_stride=1,norm_cfg =norm_cfg ,indice_key=indice_key,
                           skip_loss=False, enlarge_voxel_channels=-1,
                            mask_multi=True, topk=True, threshold=0.5)
...
class SparseBasicBlock1(spconv.SparseModule):
    def __init__(
        ...
        self.conv1 = conv1x1(inplanes, midplanes,norm_cfg, stride, indice_key=indice_key + "1",norm_cfg=norm_cfg,  bias=bias)
        ...
        self.relu = nn.ReLU()
        self.conv2 = conv3x3(midplanes, midplanes,norm_cfg, indice_key=indice_key + "1",norm_cfg=norm_cfg, bias=bias)
        ...
        self.conv3 = conv1x1(midplanes, planes,norm_cfg, indice_key=indice_key + "2",norm_cfg=norm_cfg, bias=bias)
        ...
        if downsample or inplanes != planes:
            self.downsample = conv1x1(inplanes, planes,norm_cfg, indice_key=indice_key + "1",norm_cfg=norm_cfg, bias=bias)
...
class SparseBasicBlock2(spconv.SparseModule):
    def __init__(
        ...
        self.conv1 = conv311(inplanes, midplanes,norm_cfg, stride, indice_key=indice_key+"1",norm_cfg=norm_cfg,  bias=bias)
        self.bn1 = build_norm_layer(norm_cfg, midplanes)[1]
        self.relu = nn.ReLU()
        # self.conv2 = conv3x3(midplanes, midplanes, indice_key=indice_key + "1",norm_cfg=norm_cfg, bias=bias)
        self.conv2 = conv3x3(midplanes, midplanes,norm_cfg, indice_key=indice_key+"2",norm_cfg=norm_cfg, bias=bias)
        self.bn2 = build_norm_layer(norm_cfg, midplanes)[1]
        # self.conv3 = conv3x3(midplanes, planes, indice_key=indice_key + "2",norm_cfg=norm_cfg, bias=bias)
        self.conv3 = conv1x1(midplanes, planes,norm_cfg, indice_key=indice_key,norm_cfg=norm_cfg, bias=bias)
        self.bn3 = build_norm_layer(norm_cfg, planes)[1]
        if downsample or inplanes != planes:
            # self.downsample = conv3x3(inplanes, planes, indice_key=indice_key + "3",norm_cfg=norm_cfg, bias=bias)
            self.downsample = conv1x1(inplanes, planes,norm_cfg, indice_key=indice_key,norm_cfg=norm_cfg, bias=bias)
            self.bn4 = build_norm_layer(norm_cfg, planes)[1]
        else:
            self.downsample = None
...
class SpMiddleResNetFHDFocal(nn.Module):
    def __init__(
        ...
        self.conv_input = spconv.SparseSequential(
            ...
            # if 0 in special_conv_list else None,
        )
        ...
        self.conv3 = SparseSequentialBatchdict(
            ...
            SparseBasicBlock2(512, 128,512, norm_cfg=norm_cfg, indice_key="res2"),
            SparseBasicBlock2(512, 128,512, norm_cfg=norm_cfg, indice_key="res2"),
    ...
    def forward(self, x, batch_dict = {},fuse_func=None):
        ...
        # [N, num_classes]
        return cls_score + loss_box_of_pts
