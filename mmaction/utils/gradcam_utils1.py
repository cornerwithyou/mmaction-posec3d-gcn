...
class GradCAM:
    ...
    def _alpha_blending(self, localization_map: torch.Tensor,
        ...
        # 改一下合并17通道
        curr_inp = curr_inp.max(dim=-1, keepdim=True).values
        curr_inp = curr_inp.repeat(1, 1, 1, 1, 3)
        #curr_inp = curr_inp*1.75#torch.norm(curr_inp, dim=(2,3), keepdim=True)

        heatmap = F.interpolate(heatmap, scale_factor=(8.0, 8.0, 1.0), mode='trilinear',align_corners=False)
       ...
       # heatmap = heatmap.permute(1,0,4,2,3)
        curr_inp = curr_inp.permute(1,0,4,2,3)
        #heatmap = [heatmap[i] for i in range(heatmap.shape[0])]
        curr_inp = [curr_inp[i] for i in range(curr_inp.shape[0])]
        for i in range(len(curr_inp)):
            #heatmap[i]  = F.interpolate(heatmap[i],scale_factor=(8.0, 8.0),mode='bicubic',align_corners=False)
            curr_inp[i] = F.interpolate(curr_inp[i], scale_factor=(8.0, 8.0), mode='bilinear', align_corners=False)
        #heatmap = torch.stack(heatmap)
        curr_inp = torch.stack(curr_inp)
        curr_inp = curr_inp.permute(1,0,3,4,2)
        #heatmap = heatmap.permute(1,0,3,4,2)
